...
<%  
		try
		{
		String un = (String)session.getAttribute("sessionuser");
		String opw = request.getParameter("opw");
		String npw = request.getParameter("npw");
		Connection con = EstablishConnection.getConnection();	
		PreparedStatement ps = con.prepareStatement("select * from user where loginname='"+un+"'");
		ResultSet rs = ps.executeQuery();
			if(rs.next()==true)
			{
				if(opw.equals(rs.getString("password")))
				{
					ps = con.prepareStatement("update user set password=? where loginname='"+un+"'");
					ps.setString(1,npw);	
					ps.executeUpdate();
					con.close();
