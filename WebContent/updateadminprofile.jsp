...
<%
	...
	{
	String st="0";	
	String sun = (String)session.getAttribute("sessionadmin"); 	
	Connection con = EstablishConnection.getConnection();	
	PreparedStatement pst = con.prepareStatement("update hruser set fathername=? , email=? , mobile=? , pincode=? , address=? , qualification=? , experience=? , achievement=? , skills=? where loginname='"+sun+"'");
	...
	if(z>0)
	{
		...
		%>
<jsp:forward page="adminsettings.jsp"></jsp:forward>
<%
	}
	else
	{
